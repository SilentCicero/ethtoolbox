...
test("basic tests", async t => {
  try {
    const simpleStore = new EthContract({
      ...
      methods: [{ type: 'constructor', inputs: [{ type: 'address', name: '_master' }] }, 'set()', 'set2()', 'Set(uint256 _value)', 'Set2(uint256 _value, address indexed _sender)', 'master():(address)', 'get():(uint256)']
    });
    ...
    const masterAddress = await simpleStore.master.call();
    ...
    }).then(data => console.log('Token Balance: ', data[0])).catch(console.log);
  } catch (error) {
    t.notOk(error);
  }
});